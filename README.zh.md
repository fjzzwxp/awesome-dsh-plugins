# DeepSeek Harness (DSH) 插件精选列表 [![Awesome](https://awesome.re/badge.svg)](https://awesome.re)

[English](README.md) | 中文

> 精选 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)（`dsh`）的社区插件列表。

DeepSeek Harness 是 DeepSeek 开源的 Agent 运行时——一个可运行的编码 Agent（Web 与无头模式），其框架中一切皆为插件：模型、工具、沙箱、会话存储、UI，甚至 Agent 主循环本身。插件既可以扩展官方编码 Agent，也可以替换其核心部件，或组装出完全不同的东西。

本列表收录可通过 `dsh plugin add` 安装的社区插件（每个插件都在 `package.json` 中声明了 `dsh.bundle` manifest）。欢迎提交 [PR](#贡献)。

> [!WARNING]
> 安装插件即在本机以你的权限运行第三方代码——它可以读取你的文件、使用你的凭据、访问网络。工具审批机制不会为插件代码提供沙箱。被本列表收录**不等于**通过安全审查：安装前请检查源码，不要在不持有密钥的环境中试用陌生插件。文末有完整免责声明。

<details>
<summary><b>收录标准</b></summary>

一个条目被收录，需要满足：可用 `dsh plugin add` 安装、一行描述与实际功能相符、分类正确、处于维护状态。

仅此而已。**本列表不对插件排名、不做质量评判，也不打算这么做。** 被收录只能证明满足上述规则。

收录也非永久：仓库消失、停止维护或确认失效的条目会被移除。完整标准与评审清单见 [contributing.md](contributing.md)。
...
- [Gitofxiongxiong/dsh-tui-grok](https://github.com/Gitofxiongxiong/dsh-tui-grok) - A DeepSeek Harness (dsh) plugin.
...
- [hyzyn/dsh-plugin-kit](https://github.com/hyzyn/dsh-plugin-kit) - Plugin family for the DeepSeek Harness (DSH) Web GUI: a pnpm monorepo with a plugin template, dev kit, and one-command all-in-one bundle — Codegraph, env & secrets, MCP config, profiles, prompts, RSS digest, global search, and terminal panel.
...
- [plolpl789/dsh-raw-html-v2](https://github.com/plolpl789/dsh-raw-html-v2) - VCP visual-synesthesia plugin v2 for DeepSeek Harness (official Slot API): raw-HTML vcp-root cards, streaming render, KaTeX/Mermaid/SVG, builtin fonts.
...
- [rootkiller6788/dsh-flow](https://github.com/rootkiller6788/dsh-flow) - A pluginized microkernel architecture for building, orchestrating, and extending multi-agent systems in DeepSeek Harness.
...
- [slow-stack/dsh-mneme](https://github.com/slow-stack/dsh-mneme) - 🧠 The memory that dreams — cross-session memory for DeepSeek Harness. Offline & private, auto-consolidates in its sleep (autoDream), visualized in a memory panel.
...
- [yyh-001/DSH-X](https://github.com/yyh-001/DSH-X) - DeepSeek Harness 轻量 Windows 启动器。选一个版本，启动 dsh web。
...
- [Amakurai/dsh-liketavern](https://github.com/Amakurai/dsh-liketavern) - A DeepSeek Harness (dsh) plugin.
...
- [LuxUmbra697/DSH-Desktop](https://github.com/LuxUmbra697/DSH-Desktop) - 把 DeepSeek Harness 装进一个独立窗口：双击即用、免终端、免浏览器，支持插件改造，载荷可裁到 210 MB（便携包 74.8 MB），内置运行环境探测与 DSH 就地更新。
...
- [whDonline/dsh-capsule](https://github.com/whDonline/dsh-capsule) - A DeepSeek Harness (dsh) plugin.
...
- ⭐ [DSH-EAC/DSH-Desktop-EAC](https://github.com/DSH-EAC/DSH-Desktop-EAC) - DeepSeek Harness Desktop (dsh-desktop). EAC: Embracing All Creation (揽尽万象). Bundled Node.js runtime with full dsh-CLI kernel, one-click startup, 10 built-in UI themes.
- ⭐ [zouyuxuan122/DSH-Desktop-EAC](https://github.com/zouyuxuan122/DSH-Desktop-EAC) - DeepSeek Harness Desktop (dsh-desktop). EAC: Embracing All Creation (揽尽万象). Bundled Node.js runtime with full dsh-CLI kernel, one-click startup, 10 built-in UI themes.
...
- [hikarioyama/Smart-DSH](https://github.com/hikarioyama/Smart-DSH) - Unofficial DeepSeek Harness plugin: mobile-friendly UI, Web Push notifications, and Linux Tailscale setup guide.
...
- [mrRisega/dsh-remote](https://github.com/mrRisega/dsh-remote) - 公网远程控制 DeepSeek Harness（dsh web）：安装即得专属加密地址，人在外面也能用手机远程访问，无需同一局域网/WiFi、无内网穿透，可选自建服务。Remote control DeepSeek Harness (dsh web) from anywhere — encrypted public URL, no LAN required.
...
**两个 README 均由脚本生成，请勿手工编辑。** 列表数据在 `data/plugins/`，一个插件一个 YAML 文件。新增插件：

```yaml
url: https://github.com/owner/repo        # 必须与仓库完全一致
name: owner/repo                          # 列表中显示的链接文字
category: ui                              # 分类见 contributing.md
description:
...
只有 `description.en` 是必填的。完整规则、分类列表与 monorepo 说明见 [contributing.md](contributing.md)。
...
被本列表收录**不代表**插件安全、可信或维护良好。安装插件即以你的权限在本机执行第三方代码——它可能读取你的文件、使用你的凭据、访问网络。工具审批机制不会为插件代码提供沙箱。安装前请务必审查源码，切勿在持有敏感凭据的机器上安装陌生插件。若发现列表中的插件有恶意行为，请提交 Issue 以便将其移除。

---

<p align="center"><sub>由 <code>data/plugins/*.yml</code> + <code>scripts/generate-readme.mjs</code> 维护 · 一插件一文件，永不冲突 · 列表自动生成，灵魂手工书写</sub></p>
